...
require_once($CFG->dirroot . '/mod/hsuforum/tests/generator_trait.php');

/**
 * Class for unit testing mod_hsuforum/activity_custom_completion.
 *
 * @package   mod_hsuforum
 * @copyright Simey Lameze <simey@moodle.com>
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
...
class custom_completion_test extends advanced_testcase {
    ...
    public function test_get_state(string $rule, int $rulecount, int $available, ?int $discussions, ?int $replies,
        ...
        $hsuforumgenerator = $this->getDataGenerator()->get_plugin_generator('mod_hsuforum');
        ...
        $cm = get_coursemodule_from_instance('hsuforum', $hsuforum->id);

        if ($rulecount > 0) {
            if ($rule == 'completiondiscussions') {
                // Create x number of discussions.
                for ($i = 0; $i < $rulecount; $i++) {
                    $hsuforumgenerator->create_discussion((object) [
                ...
                [$discussion1, $post1] = $this->helper_post_to_hsuforum($hsuforum, $student);
                for ($i = 0; $i < $rulecount; $i++) {
                    $this->helper_reply_to_post($post1, $student);
                }
            } else if ($rule == 'completionposts') {
                for ($i = 0; $i < $rulecount; $i++) {
                    $this->helper_post_to_hsuforum($hsuforum, $student);
                }
            }
        }
