...
class mod_hsuforum_generator extends testing_module_generator {
    ...
    public function create_discussion($record = null) {
        ...
        if (property_exists($record, 'tags')) {
            $cm = get_coursemodule_from_instance('hsuforum', $record->forum);
            $tags = is_array($record->tags) ? $record->tags : preg_split('/,/', $record->tags);

            core_tag_tag::set_item_tags('mod_hsuforum', 'hsuforum_posts', $post->id,
    ...
    public function create_post($record = null) {
        ...
        if (property_exists($record, 'tags')) {
            $discussion = $DB->get_record('hsuforum_discussions', ['id' => $record->discussion]);
            $cm = get_coursemodule_from_instance('hsuforum', $discussion->forum);
            $tags = is_array($record->tags) ? $record->tags : preg_split('/,/', $record->tags);

            core_tag_tag::set_item_tags('mod_hsuforum', 'hsuforum_posts', $record->id,
                context_module::instance($cm->id), $tags);
        }
