...
use mod_hsuforum\local\vaults\db_table_vault;
use mod_hsuforum\local\vaults\preprocessors\extract_record as extract_record_preprocessor;
use mod_hsuforum\local\vaults\preprocessors\extract_user as extract_user_preprocessor;
...
class discussion_list extends db_table_vault {
    ...
    private const TABLE = 'hsuforum_discussions';
    ...
    protected function generate_get_records_sql(string $wheresql = null, ?string $sortsql = null, ?int $userid = null): string {
        ...
        $posttable = new dml_table('hsuforum_posts', 'fp', 'p_');
        ...
        if ($issortbyreplies) {
            ...
            $tables .= ' JOIN (
                            ...
                            FROM {hsuforum_discussions} rd
                            LEFT JOIN {hsuforum_posts} rp
    ...
    protected function get_preprocessors(): array {
        return array_merge(
            ...
            [
                ...
                'firstpost' => new extract_record_preprocessor('hsuforum_posts', 'p_'),
    ...
    public function get_total_discussion_count_from_forum_id(
        ...
        $wheresql = "{$alias}.hsuforum = :forumid";
    ...
    public function get_total_discussion_count_from_forum_id_and_group_id(
        ...
        $wheresql = "{$alias}.hsuforum = :forumid AND ";
        $groupparams = [];
        if (empty($groupids)) {
            $wheresql .= "{$alias}.groupid = :allgroupsid";
        } else {
    ...
    private function get_favourite_sql(int $userid): array {
        ...
        list($favsql, $favparams) = $ufservice->get_join_sql_by_type('mod_hsuforum', 'discussions',
