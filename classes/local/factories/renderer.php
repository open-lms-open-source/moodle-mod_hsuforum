...
use mod_hsuforum\local\factories\entity as entity_factory;
use mod_hsuforum\local\factories\exporter as exporter_factory;
use mod_hsuforum\local\factories\manager as manager_factory;
use mod_hsuforum\local\factories\builder as builder_factory;
use mod_hsuforum\local\factories\url as url_factory;
use mod_hsuforum\local\renderers\discussion as discussion_renderer;
use mod_hsuforum\local\renderers\discussion_list as discussion_list_renderer;
use mod_hsuforum\local\renderers\posts as posts_renderer;
use moodle_page;
use core\output\notification;
...
class renderer {
    ...
    public function __construct(
        ...
        $this->page = $page;
        $this->rendererbase = $page->get_renderer('mod_hsuforum');
    }

    /**
     * Create a discussion renderer for the given hsuforum and discussion.
     *
    ...
    public function get_posts_renderer(): posts_renderer {
        return new posts_renderer(
            ...
            'mod_hsuforum/forum_discussion_posts'
        );
    }
    ...
    public function get_single_discussion_posts_renderer(int $displaymode = null, bool $readonly = false): posts_renderer {
        ...
        switch ($displaymode) {
            case HSUFORUM_MODE_THREADED:
                $template = 'mod_hsuforum/forum_discussion_threaded_posts';
                break;
            case HSUFORUM_MODE_NESTED:
                $template = 'mod_hsuforum/forum_discussion_nested_posts';
                break;
            case HSUFORUM_MODE_NESTED_V2:
                $template = 'mod_hsuforum/forum_discussion_nested_v2_posts';
                break;
            default;
                $template = 'mod_hsuforum/forum_discussion_posts';
                break;
        }
            ...
            function($exportedposts, $forums, $discussions) use ($displaymode, $readonly, $exportedpostssorter) {
                ...
                $exportedposts = array_map(
                    function($exportedpost) use ($forum, $discussionsbyid, $readonly, $seenfirstunread, $displaymode) {
                        ...
                        if ($displaymode === HSUFORUM_MODE_NESTED_V2) {
                ...
                if (
                    ...
                    $displaymode === HSUFORUM_MODE_THREADED ||
                    $displaymode === HSUFORUM_MODE_NESTED_V2
                ) {
                            ...
                            if (empty($replies)) {
                                ...
                                $sortedreplies = array_map(function($reply) use ($post) {
                                    if (isset($post->author)) {
                                        ...
                                        $reply->parentauthorname = get_string('firstpost', 'mod_hsuforum');
                                    }
                                    return $reply;
                                }, $sortedreplies);
                ...
                } else if ($displaymode === HSUFORUM_MODE_FLATNEWEST || $displaymode === HSUFORUM_MODE_FLATOLDEST) {
    ...
    /**
     * Create a posts renderer to render posts in the hsuforum search results.
     *
    ...
    public function get_posts_search_results_renderer(array $searchterms): posts_renderer {
        ...
        return new posts_renderer(
            $this->rendererbase,
            $this->builderfactory->get_exported_posts_builder(),
            'mod_hsuforum/forum_search_results',
    ...
    /**
     * Create a posts renderer to render posts in mod/hsuforum/user.php.
     *
     * @param bool $addlinkstocontext Should links to the course, hsuforum, and discussion be included?
     * @return posts_renderer
     */
    public function get_user_forum_posts_report_renderer(bool $addlinkstocontext): posts_renderer {
        ...
        return new posts_renderer(
            $this->rendererbase,
            $this->builderfactory->get_exported_posts_builder(),
            'mod_hsuforum/forum_posts_with_context_links',
        ...
        switch ($forum->get_type()) {
            case 'news':
                if (SITEID == $forum->get_course_id()) {
                    $template = 'mod_hsuforum/frontpage_news_discussion_list';
                } else {
                    $template = 'mod_hsuforum/news_discussion_list';
                }
                break;
            case 'qanda':
                $template = 'mod_hsuforum/qanda_discussion_list';
                break;
            default:
                $template = 'mod_hsuforum/discussion_list';
        }
    ...
    public function get_blog_discussion_list_renderer(
        ...
        return $this->get_detailed_discussion_list_renderer($forum, 'mod_hsuforum/blog_discussion_list');
    }
    ...
    public function get_social_discussion_list_renderer(
        ...
        return $this->get_detailed_discussion_list_renderer($forum, 'mod_hsuforum/social_discussion_list');
    }
    ...
    public function get_frontpage_news_discussion_list_renderer(
        ...
        return $this->get_detailed_discussion_list_renderer($forum, 'mod_hsuforum/frontpage_social_discussion_list');
    }
    ...
    public function get_single_discussion_list_renderer(
        ...
        if ($hasmultiplediscussions) {
            $notifications[] = (new notification(get_string('warnformorepost', 'hsuforum')))
