...
/**
 * A URL factory for the hsuforum of today.
 *
...
/**
 * A URL factory for the hsuforum.
 *
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class url {
    ...
    public function get_discussion_create_url(forum_entity $forum): moodle_url {
        return new moodle_url('/mod/hsuforum/post.php', [
            'forum' => $forum->get_id(),
        ]);
    }
    ...
    public function get_discussion_view_url_from_discussion_id(int $discussionid): moodle_url {
        return new moodle_url('/mod/hsuforum/discuss.php', [
            'd' => $discussionid
        ]);
    }
    ...
    public function get_reply_to_post_url_from_post(post_entity $post): moodle_url {
        return new moodle_url('/mod/hsuforum/post.php#mformforum', [
            'reply' => $post->get_id()
        ]);
    }
    ...
    public function get_export_post_url_from_post(post_entity $post): ?moodle_url {
        ...
        $button->set_callback_options('forum_portfolio_caller', ['postid' => $post->get_id()], 'mod_hsuforum');
     ...
     * @return moodle_url
     */
    public function get_mark_post_as_read_url_from_post(post_entity $post, int $displaymode = HSUFORUM_MODE_THREADED): moodle_url {
        $params = [
        ...
        $url = new moodle_url('/mod/hsuforum/discuss.php', $params);
     ...
     *
     * @param post_entity $post The post
     * @param int $displaymode The display mode to show the hsuforum in after marking as unread
     * @return moodle_url
     */
    public function get_mark_post_as_unread_url_from_post(post_entity $post, int $displaymode = HSUFORUM_MODE_THREADED): moodle_url {
        $params = [
            'd' => $post->get_discussion_id(),
            'postid' => $post->get_id(),
            'mark' => 'unread'
    ...
    public function get_mark_discussion_as_read_url_from_discussion(
        ...
        return new moodle_url('/mod/hsuforum/markposts.php', [
    ...
    public function get_discussion_subscribe_url(discussion_entity $discussion): moodle_url {
        return new moodle_url('/mod/hsuforum/subscribe.php', [
            'sesskey' => sesskey(),
    ...
    public function get_pin_discussion_url_from_discussion(discussion_entity $discussion): moodle_url {
        return new moodle_url('discuss.php', [
            'sesskey' => sesskey(),
            'd' => $discussion->get_id(),
            'pin' => $discussion->is_pinned() ? HSUFORUM_DISCUSSION_UNPINNED : HSUFORUM_DISCUSSION_PINNED
        ]);
    }
}
