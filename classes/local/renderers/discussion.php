...
namespace mod_hsuforum\local\renderers;
...
use mod_hsuforum\local\entities\sorter as sorter_entity;
...
use mod_hsuforum\local\managers\capability as capability_manager;
...
class discussion {
    ...
    public function render(
        ...
        if (!$capabilitymanager->can_view_discussions($user)) {
            throw new moodle_exception('noviewdiscussionspermission', 'mod_hsuforum');
        }
        ...
        if (($warningobj = hsuforum_check_throttling($this->forumrecord, $cmrecord)) && $warningobj->canpost) {
        ...
        if ($this->displaymode === HSUFORUM_MODE_NESTED_V2) {
            $template = 'mod_hsuforum/forum_discussion_nested_v2';
        } else {
            $template = 'mod_hsuforum/forum_discussion';
        }
    ...
    private function get_display_mode_selector_html(int $displaymode, stdClass $user): string {
        ...
        $select->set_label(get_string('displaymode', 'hsuforum'), ['class' => 'accesshide']);
    ...
    private function get_move_discussion_html(): ?string {
        ...
        if (isset($modinfo->instances['hsuforum'])) {
            ...
            $forumcheck = $DB->get_records('hsuforum', ['course' => $courseid], '', 'id, type');
            foreach ($modinfo->instances['hsuforum'] as $forumcm) {
                ...
                if ($forumidcompare and $forumtypecheck) {
                    $url = "/mod/hsuforum/discuss.php?d={$discussion->get_id()}&move=$forumcm->instance&sesskey=".sesskey();
    ...
    private function get_export_discussion_html(stdClass $user): ?string {
        ...
        $button->set_callback_options('forum_portfolio_caller', ['discussionid' => $this->discussion->get_id()], 'mod_hsuforum');
    ...
    private function get_notifications($user): array {
        ...
        if ($forum->is_cutoff_date_reached()) {
            $notifications[] = (new notification(
                    get_string('cutoffdatereached', 'hsuforum'),
            ...
            if ($forum->is_due_date_reached()) {
                $notifications[] = (new notification(
                    get_string('thisforumisdue', 'hsuforum', userdate($forum->get_due_date())),
                    notification::NOTIFY_INFO
                ))->set_show_closebutton();
            } else if ($forum->has_due_date()) {
                $notifications[] = (new notification(
                    get_string('thisforumhasduedate', 'hsuforum', userdate($forum->get_due_date())),
                    notification::NOTIFY_INFO
                ))->set_show_closebutton();
            }
        }
        ...
        if ($forum->is_discussion_locked($discussion)) {
            $notifications[] = (new notification(
                get_string('discussionlocked', 'hsuforum'),
                notification::NOTIFY_INFO
            ))
        ...
        if ($forum->get_type() == 'qanda') {
            if ($this->capabilitymanager->must_post_before_viewing_discussion($user, $discussion)) {
                $notifications[] = (new notification(
                    get_string('qandanotify', 'hsuforum')
        ...
        if ($forum->has_blocking_enabled()) {
            $notifications[] = (new notification(
                get_string('thisforumisthrottled', 'hsuforum', [
    ...
    private function get_neighbour_links_html(): string {
        ...
        $neighbours = hsuforum_get_discussion_neighbours($coursemodule, $this->discussionrecord, $this->forumrecord);
