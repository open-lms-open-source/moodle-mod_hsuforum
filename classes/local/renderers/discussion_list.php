...
class discussion_list {
    ...
    public function render(
        ...
        $alldiscussionscount = mod_hsuforum_count_all_discussions($forum, $user, $groupid);

        // Get all forum discussion summaries.
        $discussions = mod_hsuforum_get_discussion_summaries($forum, $user, $groupid, $sortorder, $pageno, $pagesize);
        ...
        $forumview = [
            ...
            'experimentaldisplaymode' => $displaymode == HSUFORUM_MODE_NESTED_V2,
    ...
    public function render_new_discussion(stdClass $user, ?int $groupid): string {
        ...
        return $this->renderer->render_from_template('mod_hsuforum/forum_new_discussion_actionbar', $forumview);
    }

    /**
     * Get the mod_hsuforum_post_form. This is the default boiler plate from mod_hsuforum/post_form.php with the inpage flag caveat
     *
    ...
    private function get_discussion_form(stdClass $user, \cm_info $cm, ?int $groupid) {
        ...
        $thresholdwarning = hsuforum_check_throttling($forumrecord, $cm);
        ...
        $formparams = array(
            ...
            'forum' => $forumrecord,
            'post' => $post,
            'subscribe' => \mod_hsuforum\subscriptions::is_subscribed($user->id, $forumrecord,
                null, $cm),
    ...
    private function get_notifications(stdClass $user, ?int $groupid): array {
        ...
        if ($forum->is_in_group_mode() && !$capabilitymanager->can_access_all_groups($user)) {
            if ($groupid === null) {
                if (!$capabilitymanager->can_post_to_my_groups($user)) {
                    $notifications[] = (new notification(
                        get_string('cannotadddiscussiongroup', 'mod_hsuforum'),
                        \core\output\notification::NOTIFY_WARNING
                    ))->set_show_closebutton();
                } else {
                    $notifications[] = (new notification(
                        get_string('cannotadddiscussionall', 'mod_hsuforum'),
                        \core\output\notification::NOTIFY_WARNING
                    ))->set_show_closebutton();
                }
            } else if (!$capabilitymanager->can_access_group($user, $groupid)) {
                $notifications[] = (new notification(
                    get_string('cannotadddiscussion', 'mod_hsuforum'),
                    \core\output\notification::NOTIFY_WARNING
                ))->set_show_closebutton();
            }
        }
        ...
        if ('eachuser' === $forum->get_type()) {
            $notifications[] = (new notification(
                get_string('allowsdiscussions', 'hsuforum'),
                notification::NOTIFY_INFO)
            )->set_show_closebutton();
        }
