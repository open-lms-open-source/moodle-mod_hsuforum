...
/**
 * Capability manager for the hsuforum.
 *
 * @package    mod_hsuforum
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */

namespace mod_hsuforum\local\managers;

defined('MOODLE_INTERNAL') || die();

use mod_hsuforum\local\data_mappers\legacy\forum as legacy_forum_data_mapper;
use mod_hsuforum\local\data_mappers\legacy\discussion as legacy_discussion_data_mapper;
use mod_hsuforum\local\data_mappers\legacy\post as legacy_post_data_mapper;
...
use mod_hsuforum\subscriptions;
use context;
use context_system;
use stdClass;
...
class capability {
    ...
    /**
     * Can the user subscribe to this hsuforum?
     *
    ...
    public function can_create_discussions(stdClass $user, int $groupid = null): bool {
        ...
        if ($this->forum->is_cutoff_date_reached()) {
            if (!has_capability('mod/hsuforum:canoverridecutoff', $this->get_context())) {
                return false;
            }
        }
        ...
        if ($warningobj = hsuforum_check_throttling($this->forumrecord, $cmrecord)) {
            return $warningobj->canpost;
        }

        switch ($this->forum->get_type()) {
            case 'news':
                $capability = 'mod/hsuforum:addnews';
                break;
            case 'qanda':
                $capability = 'mod/hsuforum:addquestion';
                break;
            default:
                $capability = 'mod/hsuforum:startdiscussion';
        }
        ...
        if ($this->forum->get_type() == 'eachuser') {
            if (hsuforum_user_has_posted_discussion($this->forum->get_id(), $user->id, $groupid)) {
                return false;
            }
        }
    ...
    public function can_post_to_my_groups(stdClass $user): bool {
        return has_capability('mod/hsuforum:canposttomygroups', $this->get_context(), $user);
    }
    ...
    public function can_view_discussions(stdClass $user): bool {
        return has_capability('mod/hsuforum:viewdiscussion', $this->get_context(), $user);
    }
    ...
    public function can_move_discussions(stdClass $user): bool {
        ...
        return $forum->get_type() !== 'single' &&
                has_capability('mod/hsuforum:movediscussions', $this->get_context(), $user);
    }
    ...
    public function can_pin_discussions(stdClass $user): bool {
        return $this->forum->get_type() !== 'single' &&
                has_capability('mod/hsuforum:pindiscussions', $this->get_context(), $user);
    }
    ...
    public function can_split_discussions(stdClass $user): bool {
        $forum = $this->get_forum();
        return $forum->get_type() !== 'single' && has_capability('mod/hsuforum:splitdiscussions', $this->get_context(), $user);
    }
    ...
    public function can_export_discussions(stdClass $user): bool {
        global $CFG;
        return $CFG->enableportfolios && has_capability('mod/hsuforum:exportdiscussion', $this->get_context(), $user);
    }
    ...
    public function must_post_before_viewing_discussion(stdClass $user, discussion_entity $discussion): bool {
        ...
        if ($forum->get_type() === 'qanda') {
            ...
            return !has_capability('mod/hsuforum:viewqandawithoutposting', $this->get_context(), $user) &&
                !hsuforum_user_has_posted($forum->get_id(), $discussion->get_id(), $user->id);
        } else {
    ...
    public function can_post_in_discussion(stdClass $user, discussion_entity $discussion): bool {
        ...
        $status = hsuforum_user_can_post($forumrecord, $discussionrecord, $user, $coursemodule, $course, $context);

        // If the user reaches the number of posts equal to warning/blocking setting then logically and canpost value with $status.
        if ($warningobj = hsuforum_check_throttling($forumrecord, $coursemodule)) {
    ...
    public function can_favourite_discussion(stdClass $user): bool {
        $context = $this->get_context();
        return has_capability('mod/hsuforum:cantogglefavourite', $context, $user);
    }
    ...
    public function can_view_discussion(stdClass $user, discussion_entity $discussion): bool {
        ...
        return hsuforum_user_can_see_discussion($forumrecord, $discussionrecord, $context, $user);
    }
    ...
    public function can_view_post(stdClass $user, discussion_entity $discussion, post_entity $post): bool {
        ...
        $coursemodule = $forum->get_course_module_record();
        $canviewpost = hsuforum_user_can_see_post($forumrecord, $discussionrecord, $postrecord, $user, $coursemodule, false);
    ...
    public function can_view_any_private_reply(stdClass $user): bool {
        return has_capability('mod/hsuforum:readprivatereplies', $this->get_context(), $user);
    }
    ...
    public function can_edit_post(stdClass $user, discussion_entity $discussion, post_entity $post): bool {
        ...
        return ($ownpost && $ineditingtime && !$mailnow) || has_capability('mod/hsuforum:editanypost', $context, $user);
    }
    ...
    public function validate_delete_post(stdClass $user, discussion_entity $discussion, post_entity $post,
        ...
        if (!($ownpost && $ineditingtime && has_capability('mod/hsuforum:deleteownpost', $context, $user) && !$mailnow ||
                has_capability('mod/hsuforum:deleteanypost', $context, $user))) {
        ...
        if ($hasreplies && !has_capability('mod/hsuforum:deleteanypost', $context, $user)) {
    ...
    public function can_reply_privately_to_post(stdClass $user, post_entity $post): bool {
        ...
        return has_capability('mod/hsuforum:postprivatereply', $this->get_context(), $user);
    }
    ...
    public function can_export_post(stdClass $user, post_entity $post): bool {
        global $CFG;
        $context = $this->get_context();
        return $CFG->enableportfolios  && (has_capability('mod/hsuforum:exportpost', $context, $user) ||
            ($post->is_owned_by_user($user) && has_capability('mod/hsuforum:exportownpost', $context, $user)));
    }

    /**
     * Get the hsuforum entity for this capability manager.
     *
     * @return forum_entity
     */
    ...
    public function can_view_hidden_posts(stdClass $user): bool {
        return has_capability('mod/hsuforum:viewhiddentimedposts', $this->get_context(), $user);
    }
    ...
    public function can_export_forum(stdClass $user): bool {
        return has_capability('mod/hsuforum:exportforum', $this->get_context(), $user);
    }
    ...
    public function can_grade(stdClass $grader, stdClass $gradee = null): bool {
        if (!has_capability('mod/hsuforum:grade', $this->get_context(), $grader)) {
            return false;
        }
