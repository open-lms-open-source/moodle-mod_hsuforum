...
class post extends \core_search\base_mod {
    ...
    public function get_document($record, $options = array()) {
        ...
        try {
            $cm = $this->get_cm('hsuforum', $record->forumid, $record->courseid);
    ...
    public function check_access($id) {
        ...
        try {
            ...
            $cminfo = $this->get_cm('hsuforum', $forum->id, $forum->course);
    ...
    protected function get_post($postid) {
        if (empty($this->postsdata[$postid])) {
            $this->postsdata[$postid] = hsuforum_get_post_full($postid);
            if (!$this->postsdata[$postid]) {
                throw new \dml_missing_record_exception('hsuforum_posts');
            }
        }
    ...
    protected function get_forum($forumid) {
        ...
        if (empty($this->forumsdata[$forumid])) {
            $this->forumsdata[$forumid] = $DB->get_record('hsuforum', array('id' => $forumid), '*', MUST_EXIST);
        }
    ...
    protected function get_discussion($discussionid) {
        ...
        if (empty($this->discussionsdata[$discussionid])) {
            $this->discussionsdata[$discussionid] = $DB->get_record('hsuforum_discussions',
